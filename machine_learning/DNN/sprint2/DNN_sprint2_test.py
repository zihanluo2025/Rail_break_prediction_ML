...
aggregated_features = input_data.groupby('p_key')[feature_columns].agg(['min', 'max', 'mean'])
...
final_data = aggregated_features.merge(
    input_data[['p_key'] + ['Tc_BaseCode_Mapped']].drop_duplicates(),
    on=['p_key'],
    how='inner'
)

X_test = final_data.drop('p_key', axis=1)
...
unique_pkeys = input_data['p_key'].unique()
print(f"p_key counts: {len(unique_pkeys)}")
