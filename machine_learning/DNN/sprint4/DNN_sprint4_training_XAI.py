...
# MAGIC %pip install pandas scikit-learn mlflow-skinny[databricks] torch torchvision torchaudio optuna shap
...
print("=" * 80)
print("Table 1: Global Feature Impact Analysis")
print("=" * 80)
...
# Analyze the relationship between each feature value and SHAP value to find ranges that lead to high fracture probability
risk_ranges = []
for i, feature in enumerate(importance_df['Feature']):
    feature_idx = feature_columns.index(feature)
    feature_values = X_test_df.iloc[:100, feature_idx]
    feature_shap = shap_values_array[:, feature_idx]
    
    # Find samples with positive SHAP values (increasing fracture probability)
    positive_shap_indices = np.where(feature_shap > 0)[0]
    if len(positive_shap_indices) > 0:
        risk_values = feature_values.iloc[positive_shap_indices]
        if len(risk_values) > 0:
            min_val = risk_values.min()
            max_val = risk_values.max()
            risk_range = f"{min_val:.3f} ~ {max_val:.3f}"
        else:
            risk_range = "Insufficient data"
    else:
        risk_range = "No clear risk range"
    
    risk_ranges.append(risk_range)

importance_df['High-Risk Range'] = risk_ranges
print(importance_df.to_string(index=False))

print("\n" + "=" * 80)
print("Table 2: Single Prediction Explanations (High-Risk Samples)")
print("=" * 80)

# Calculate prediction probabilities for all test samples
all_pred_probs = model_predict(X_test_scaled[:100])

# Find samples with high fracture probability (>= 0.5)
high_risk_indices = np.where(all_pred_probs >= 0.5)[0]

# If not enough high-risk samples, use the ones with highest probability
if len(high_risk_indices) < 3:
    # Get indices of top 3 highest probability samples
    high_risk_indices = np.argsort(all_pred_probs)[-3:][::-1]
    print(f"Note: Only {len(np.where(all_pred_probs >= 0.5)[0])} samples with probability >= 0.5 found.")
    print(f"Using top {len(high_risk_indices)} highest probability samples instead.")

# Analyze high-risk samples
for i, sample_idx in enumerate(high_risk_indices[:3]):  # Limit to 3 samples
    print(f"\n--- Prediction Explanation for High-Risk Sample {i+1} ---")
    
    sample_shap = shap_values_array[sample_idx]
    sample_features = X_test_df.iloc[sample_idx]
    
    # Get prediction probability for this sample
    pred_prob = all_pred_probs[sample_idx]
    
    print(f"Predicted fracture probability: {pred_prob:.4f}")
    
    # Create explanation for all features, sorted by absolute SHAP value
    sample_explanation = []
    ...
    sample_explanation_df = pd.DataFrame(sample_explanation)
    sample_explanation_df['Abs SHAP'] = np.abs(sample_explanation_df['SHAP Value'])
    sample_explanation_df = sample_explanation_df.sort_values('Abs SHAP', ascending=False)
    
    # Create table with feature impact direction
    impact_direction = []
    for shap_val in sample_explanation_df['SHAP Value']:
        if shap_val > 0:
            impact_direction.append("Increases fracture risk")
        elif shap_val < 0:
            impact_direction.append("Decreases fracture risk")
        else:
            impact_direction.append("No impact")
    
    single_pred_df = pd.DataFrame({
        'Feature': sample_explanation_df['Feature'],
        'Impact': impact_direction,
        'Feature Value': sample_explanation_df['Feature Value'].apply(lambda x: f"{x:.3f}"),
        'SHAP Value': sample_explanation_df['SHAP Value'].apply(lambda x: f"{x:.6f}")
    })
    
    print(single_pred_df.head(10).to_string(index=False))  # Show top 10 features
