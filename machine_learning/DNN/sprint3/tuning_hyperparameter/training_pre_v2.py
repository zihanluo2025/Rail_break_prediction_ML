...
'''
# count and show the number of missing value
missing_values_per_column = full_data.select(
    [sum(isnull(col(c)).cast("int")).alias(c) for c in full_data.columns]
)
missing_values_per_column.show(vertical=True)

tc_target_missing = full_data.filter(col("Tc_target").isNull()).count()
total_records = full_data.count()
print(f"missing Tc_target: {tc_target_missing}")
print(f"Tc_target missing proportion: {(tc_target_missing / total_records) * 100:.2f}%")
'''
...
# rebalance the two classes in the dataset
...
criterion = nn.CrossEntropyLoss(weight=class_weights) # apply class weights to the loss function
