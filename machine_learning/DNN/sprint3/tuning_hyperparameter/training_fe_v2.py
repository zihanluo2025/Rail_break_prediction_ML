...
for col in feature_columns:
    agg_exprs.extend([
        ...
        F.percentile_approx(col, 0.5).alias(f"{col}_median")
    ])
...
def objective(trial, X, y):
    ...
    hidden_size1 = trial.suggest_categorical("hidden_size1", [256, 512, 1024])
    hidden_size2 = trial.suggest_categorical("hidden_size2", [128, 256, 512])
    hidden_size3 = trial.suggest_categorical("hidden_size3", [64, 128, 256])
    hidden_size4 = trial.suggest_categorical("hidden_size4", [32, 64, 128])
    ...
    class EnhancedDNN(nn.Module):
        def __init__(self, input_dim, use_residual):
            ...
            self.output_layer = nn.Linear(hidden_size4, 2)
            self.relu = nn.ReLU()
            
            if use_residual and input_dim != hidden_size4:
                self.shortcut = nn.Sequential(
                    nn.Linear(input_dim, hidden_size4),
    ...
    model = EnhancedDNN(input_dim, use_residual)
...
class FinalDNN(nn.Module):
    def __init__(self, input_dim):
        ...
        self.use_residual = best_params["use_residual"]
        
        self.layer1 = nn.Linear(input_dim, best_params["hidden_size1"])
        self.bn1 = nn.BatchNorm1d(best_params["hidden_size1"])
        self.layer2 = nn.Linear(best_params["hidden_size1"], best_params["hidden_size2"])
        self.bn2 = nn.BatchNorm1d(best_params["hidden_size2"])
        self.layer3 = nn.Linear(best_params["hidden_size2"], best_params["hidden_size3"])
        self.bn3 = nn.BatchNorm1d(best_params["hidden_size3"])
        self.layer4 = nn.Linear(best_params["hidden_size3"], best_params["hidden_size4"])
        self.bn4 = nn.BatchNorm1d(best_params["hidden_size4"])
        self.output_layer = nn.Linear(best_params["hidden_size4"], 2)
        self.relu = nn.ReLU()
        
        if self.use_residual and input_dim != best_params["hidden_size4"]:
            self.shortcut = nn.Sequential(
                nn.Linear(input_dim, best_params["hidden_size4"]),
                nn.BatchNorm1d(best_params["hidden_size4"])
            )
        else:
            self.shortcut = nn.Identity()
...
if best_model_state is not None:
    ...
    with torch.no_grad():
        ...
        print(f'Final accuracy: {final_accuracy:.4f}, Final F1 score: {final_f1:.4f}')
else:
