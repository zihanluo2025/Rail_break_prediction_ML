...
# build entire canlender
def build_calendar_and_windows(g, date_col, feature_cols, L):
    ...
    Xmat = g_full[feature_cols].to_numpy(dtype=np.float32)
    dates = g_full[date_col].to_numpy()
    return Xmat, dates

# COMMAND ----------

# build windows for inference
def windows_for_inference(df, group_key_col, date_col, feature_cols, L=30, anchor_stride=7):
    X_list, keys = [], []
    df = df.sort_values([group_key_col, date_col])

    for gk, g in df.groupby(group_key_col):
        Xmat, dates = build_calendar_and_windows(g, date_col, feature_cols, L)

        # build a window per anchor_stride days
        anchor_dates = g[date_col].to_numpy()[::max(int(anchor_stride), 1)]

        for ad in anchor_dates:
            end_pos = dates.searchsorted(ad, side="right") - 1
            if end_pos < 0:
                continue
            start = max(0, end_pos - (L - 1))
            win = Xmat[start:end_pos+1]

            if len(win) < L:
                pad_len = L - len(win)
                win = np.pad(win, ((pad_len, 0), (0, 0)), mode='edge')

            X_list.append(win)
            keys.append(f"{gk}{pd.Timestamp(ad).strftime('%Y-%m-%d')}")
