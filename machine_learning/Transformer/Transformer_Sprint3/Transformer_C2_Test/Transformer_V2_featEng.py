...
# MAGIC | ---: | :--- | :--- |
# MAGIC | 2025-09-13 | Di	Zhu | competition 2 (Transformer with fe_training table).|
...
# training model
transformer_model = Transformer(in_dim= X_train.shape[-1], d_model=256, nhead=8, num_layers=3, dropout=0.1).to(device)
criterion = nn.BCEWithLogitsLoss(pos_weight=pos_weight) 
optimizer = torch.optim.AdamW(transformer_model.parameters(), lr=3e-4, weight_decay=1e-4)
...
for epoch in range(10):
    ...
    for X_train, y_train in train_loader:
        ...
        logit = transformer_model(X_train)       
...
# training score
train_acc, train_auc, train_f1, train_preds = evaluation_metrics(transformer_model, train_loader, device)
...
# validation score
val_acc, val_auc, val_f1, val_preds = evaluation_metrics(transformer_model, val_loader, device)
...
# Testing score
accuracy, auc, f1, preds = evaluation_metrics(transformer_model, test_loader, device)
...
def make_windows_for_inference(df, group_key_col,date_col,feature_cols, L, anchors, dedup, anchor_stride):
    df = df.copy()
    ...
    for gk, g in df.sort_values([group_key_col, date_col]).groupby(group_key_col):
        ...
        if anchors == "all":
            anchor_dates = g_full[date_col].iloc[::max(int(anchor_stride), 1)].tolist()
    ...
    if not X_list:
        return np.empty((0, L, len(feature_cols)), np.float32), []
...
X3d, p_keys = make_windows_for_inference(
    df=input_data,
...
# scaler
scaler = StandardScaler()
X_3d_scaled = scaler.fit_transform(X3d.reshape(-1,D)).astype(np.float32).reshape(K,L,D)
...
with torch.no_grad():
    for s in range(0, K, batch):
        e = min(s+batch, K)
        xb = torch.from_numpy(X3d[s:e]).to(device).float()
        logit = transformer_model(xb)
