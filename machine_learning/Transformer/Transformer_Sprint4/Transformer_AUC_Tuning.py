...
# MAGIC | ---: | :--- | :--- |
# MAGIC | 2025-10-12 | Di	Zhu | Transformer with tuning hyperparameters (Metric: AUC).|
...
df = spark.sql("""
SELECT *
FROM `09ad024f-822f-48e4-9d9e-b5e03c1839a2`.predictive_maintenance_uofa_2025.preprocess_training_table LIMIT 20000
""")
...
def training_model(model, train_loader, val_loader, device, epochs=10, lr=1e-3, weight_decay = 1e-3, patience=3):
    ...
    for ep in range(1, epochs+1):
        ...
        cur_auc = roc_auc_score(y_true, y_prob)
    ...
    return {"model": model, "val_best_auc": best_val_auc}
...
def tuing_hyperparameters(train_loader, val_loader):
    ...
    for i, hp in enumerate(combos, 1):
        ...
        cur_result = {
            "hparams": hp,
            "val_best_auc": output["val_best_auc"],
            "model": output["model"],
        }

        print(f"HP {i}: hp:{hp}, AUC={cur_result['val_best_auc']:.4f}")

        if best_result is None or cur_result["val_best_auc"] > best_result["val_best_auc"]:
...
print(f'Best hyperparameters: {best["hparams"]}, Best val auc: {best["val_best_auc"]}')

# COMMAND ----------

# find the best threshold in Val
y_prob_val, y_pre_val = predict_proba(best_model, val_loader, device)

best_thr, best_f1 = 0.5, -1
for thr in np.linspace(0, 1, 101):
    y_pred = (y_prob_val >= thr).astype(int)
    f1 = f1_score(y_pre_val, y_pred)
    if f1 > best_f1:
        best_f1, best_thr = f1, thr
print("Best thr on Val:", best_thr, "Val F1:", best_f1)
...
def evaluation_metrics(model,loader, device):
    ...
    with torch.no_grad():
        for xb, yb in loader:
            ...
            logit = model(xb)       
            pred = (torch.sigmoid(logit) >= best_thr).int().reshape(-1) 
