...
# MAGIC | ---: | :--- | :--- |
# MAGIC | 2025-10-17 | Di	Zhu | SHAP for transformer.|
...
feature_columns = pandas_df.drop([
    "p_key",
...
# Data preparation
background = X_train[:64].to(device)  # (64, 30, 8)
sample = X_val[:128].to(device)       # (128, 30, 8)

# Flat data to be 2D for SHAP
background_flat = background.reshape(64, -1).float()   # (64, 240)
sample_flat = sample.reshape(128, -1).float()          # (128, 240)
print(f"Background shape: {background_flat.shape}")
print(f"Sample shape: {sample_flat.shape}")

# Check model shape
with torch.no_grad():
    test_output = model(background[:2])
    print(f"Input shape: {test_output.shape}")
    print(f"Model output shape: {test_output.shape}")

# # Convert the model input and output to be 2D for SHAP
class TransformerWrapper(nn.Module):
    def __init__(self, model, seq_len=30, n_features=len(feature_columns)):
        super().__init__()
        self.model = model
    ...
    def forward(self, x):  
        ...
        x_3d = x.view(batch_size, self.seq_len, self.n_features)
        output = self.model(x_3d)
        if output.dim() == 1:
            output = output.unsqueeze(1)
        return output

wrapped_transformer = TransformerWrapper(model).to(device).eval()
with torch.no_grad():
    test_output = wrapped_transformer(background[:5])
    print(f"Input shape: {test_output.shape}")
    print(f"Wrapped transformer output shape: {test_output.shape}")

# COMMAND ----------

# SHAP explainer
explainer = shap.GradientExplainer(wrapped_transformer, background_flat)
...
# aggregation
shap_3d = shap_values.reshape(-1, seq_len, n_features)
shap_agg = shap_3d.mean(axis=1)  
data_agg = sample_flat.reshape(-1, seq_len, n_features).mean(axis=1)
...
# top 20 features contribution
shap.plots.beeswarm(shap_explanation, max_display=20)

# COMMAND ----------

# select top 20 importance
mean_importance = np.abs(shap_agg).mean(axis=0)
...
feature_idx = [name_to_idx[f] for f in selected_features]

X_train_top = X_train[:, :, feature_idx]
X_val_top   = X_val[:, :, feature_idx]
X_test_top  = X_test[:, :, feature_idx]
print(X_train_top.shape, X_val_top.shape, X_test_top.shape)

# COMMAND ----------

train_top_dataset = TensorDataset(X_train_top, y_train)
val_top_dataset = TensorDataset(X_val_top,y_val)
test_top_dataset = TensorDataset(X_test_top,y_test)

train_top_loader = DataLoader(train_top_dataset, batch_size=batch_size, shuffle=True)
val_top_loader = DataLoader(val_top_dataset, batch_size=batch_size, shuffle=False)
test_top_loader = DataLoader(test_top_dataset, batch_size=batch_size, shuffle=False)

# COMMAND ----------

# retrain model
retrain_model = Transformer(in_dim= X_train_top.shape[-1],d_model=128,nhead=4,num_layers=2,dropout=0.4)
result = training_model(retrain_model, train_top_loader, val_top_loader, device, epochs=30, lr=5e-5, weight_decay=1e-3, patience=3)
retrained_model = result["model"]

# COMMAND ----------

# retained mode validation score
val_acc, val_auc, val_f1, val_preds = evaluation_metrics(retrained_model, val_top_loader, device)
print(f"Val accuracy = {val_acc:.4f}, Val auc = {val_auc:.4f}, Val f1 = {val_f1:.4f}")
...
# Testing score
accuracy, auc, f1, preds = evaluation_metrics(retrained_model, test_top_loader, device)
...
with mlflow.start_run() as run:
    ...
    mlflow.pytorch.log_model(retrained_model,artifact_path=model_name
