...
# MAGIC | ---: | :--- | :--- |
# MAGIC | 2025-10-07 | Di	Zhu | Competition 3: Transformer Initial version (Metric: F1).|
...
# config
save_model = True; load_model = False; model_file_name = "best_transformer.pt"
...
def training_model(model, train_loader, val_loader, device, epochs=10, lr=1e-3, weight_decay=1e-3, patience=3):
    model.to(device)
    ...
    for ep in range(1, epochs+1):
        ...
        print(f"Epoch {ep}/{epochs} | Train Loss: {avg_loss:.4f} | Val F1: {cur_f1:.4f}")
    ...
    return {"model": model, "val_best_f1": best_val_f1}

# COMMAND ----------

model = Transformer(in_dim= X_train.shape[-1],d_model=128,nhead=4,num_layers=2,dropout=0.4)

# COMMAND ----------

if not skip_training_loop:
    result = training_model(model, train_loader, val_loader, device, epochs=30, lr=5e-5, weight_decay=1e-3, patience=3)
...
if load_model and os.path.exists(model_file_name):
    trained_model = model
    trained_model.load_state_dict(torch.load(model_file_name, map_location=device))
    trained_model.eval()
...
# Val score
val_acc, val_auc, val_f1, val_preds = evaluation_metrics(trained_model, val_loader, device)
print(f"Val accuracy = {val_acc:.4f}, Val auc = {val_auc:.4f}, Val f1 = {val_f1:.4f}")
...
# Testing score
accuracy, auc, f1, preds = evaluation_metrics(trained_model, test_loader, device)
print(f"Test accuracy = {accuracy:.4f}, Test auc = {auc:.4f}, Test f1 = {f1:.4f}")

# COMMAND ----------

# MAGIC %md
# MAGIC # LIME explanation
...
if explain_LIME:
    ...
    L = X_train.shape[1]   # timesteps
    D = X_train.shape[2]   # features

    # Build feature names like t0_Wagon_Speed, t1_Wagon_Speed, ...etc.
    feature_names = []
    for t in range(L):
        for fn in feature_columns:
            feature_names.append(f"t{t}_{fn}")

    # Prepare flattened training data for LimeTabularExplainer
    train_np_flat = X_train.cpu().numpy().reshape(-1, L * D)  # flattened to shape (N_train, L*D)
    print(f"train_np_flat size: {train_np_flat.shape[0]}")
    # pick a sample subset (randomly 2000 records):
    idx = np.random.choice(train_np_flat.shape[0], size=2000, replace=False)
    explainer_train_data = train_np_flat[idx]

    # function mapping array (shape=(n, L*D)) -> probabilities shape (n,)
    # returns prob for positive class
    def lime_predict_fn(X_flat_numpy):
        model = trained_model
        model.eval()
        X_reshaped = X_flat_numpy.reshape(-1, L, D).astype(np.float32)
        xb = torch.from_numpy(X_reshaped).to(device).float()
        with torch.no_grad():
            logits = model(xb)
            pos_prob = torch.sigmoid(logits).cpu().numpy()       # shape (n,)
        neg_prob = 1 - pos_prob                                  # shape (n,)
        return np.column_stack((neg_prob, pos_prob))             # shape (n, 2)

    explainer = lime_tabular.LimeTabularExplainer(
        training_data=explainer_train_data,
        feature_names=feature_names,
        discretize_continuous=False,
        mode='classification'
    )

    # any index from the test set
    idx = 0

    test_np_flat = X_test.numpy().reshape(-1, L * D)
    instance = test_np_flat[idx]

    exp = explainer.explain_instance(
        data_row=instance,
        predict_fn=lime_predict_fn,
        num_samples=1000,   # number of perturbed samples
        num_features=30     # number of features to show in explanation
    )

    # top contributions (feature_name, contribution)
    print("Top LIME contributions (flattened features):")
    for feat, weight in exp.as_list():
        print(feat, weight)

    # Convert the LIME explanation an array of shape (L, D)
    map_for_pos = exp.as_map()[1] if 1 in exp.as_map() else exp.as_map().get(0, [])
    importance_flat = np.zeros(L * D, dtype=np.float32)
    for fid, w in map_for_pos:
        importance_flat[fid] = w

    importance_ts = importance_flat.reshape(L, D)

    # Aggregate by feature across timesteps
    per_feature_importance = np.abs(importance_ts).sum(axis=0)  # length D
    feature_importance_pairs = list(zip(feature_columns, per_feature_importance))
    feature_importance_pairs = sorted(feature_importance_pairs, key=lambda x: x[1], reverse=True)

    print("[LIME] Aggregated per-feature importance (abs-sum over timesteps):")
    for fname, impt in feature_importance_pairs:
        print(f"{fname}: {impt:.4f}")

    # Feature importances bar chart
    plt.figure(figsize=(8,4))
    names = [p[0] for p in feature_importance_pairs]
    vals = [p[1] for p in feature_importance_pairs]
    plt.bar(range(len(names)), vals)
    plt.xticks(range(len(names)), names, rotation=45, ha='right')
    plt.title('[LIME] Aggregated importance by feature (abs-sum over timesteps)')
    plt.tight_layout()
    plt.show()

    # Heatmap importance across timesteps x features
    plt.figure(figsize=(10,6))
    plt.imshow(importance_ts.T, aspect='auto')
    plt.xlabel('Timesteps')
    plt.ylabel('Features')
    plt.yticks(ticks=np.arange(D), labels=feature_columns)
...
# Heatmap importance across multiple instances
if explain_LIME:
    def explain_multiple_indices(indices, num_samples=500):
        agg = np.zeros((L, D), dtype=np.float32)
        for i in indices:
            inst = test_np_flat[i]
            e = explainer.explain_instance(inst, lime_predict_fn, num_samples=num_samples, num_features=100)
            m = np.zeros(L*D, dtype=np.float32)
            mp = e.as_map()
            class_idx = list(mp.keys())[0]
            for fid, w in mp[class_idx]:
                m[fid] = w
            agg += np.abs(m.reshape(L, D))
        agg /= len(indices)
        return agg

    # first 20 test samples
    avg_imp = explain_multiple_indices(list(range(min(20, test_np_flat.shape[0]))), num_samples=500)
    plt.figure(figsize=(10,6))
    plt.imshow(avg_imp.T, aspect='auto')
    plt.xlabel('Timesteps')
    plt.ylabel('Features')
    plt.yticks(ticks=np.arange(D), labels=feature_columns)
    plt.title('[LIME] Average absolute importance for first 20 samples')
    plt.colorbar(label='Importance')
    plt.tight_layout()
    plt.show()
...
with mlflow.start_run() as run:
    ...
    mlflow.pytorch.log_model(trained_model,artifact_path=model_name
