...
# MAGIC | ---: | :--- | :--- |
# MAGIC | 2025-09-05 | Di	Zhu | version 3 in judy_test.|
...
# build windows
def sequence_windows (X, y, seq_len):
    X_seq, y_seq = [], []
    for i in range(len(X) - seq_len+1):
        X_seq.append(X[i:i+seq_len])
        y_seq.append(y[i+seq_len-1])
    return np.array(X_seq), np.array(y_seq)
X_seq, y_seq = sequence_windows(X, y, 30)
...
# Test accuracy
accuracy, y_pred = get_accuracy(transformer_model, test_loader, device)
...
with mlflow.start_run() as run:
    ...
    sign=infer_signature(model_input=X_sig.reset_index(drop=True),model_output=y_pred)
