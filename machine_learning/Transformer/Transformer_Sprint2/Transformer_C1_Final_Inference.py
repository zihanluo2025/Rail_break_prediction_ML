...
input_data['p_key'] = (
    ...
    input_data['Tc_r_date'].dt.strftime('%Y-%m-%d')
)
final_data = (input_data.sort_values(['p_key', 'Tc_r_date'])
              .groupby('p_key')
              .tail(30)
)
...
# only get the last 30 records
counts = final_data.groupby('p_key').size()
keys = counts[counts == 30].index
final_data = final_data[final_data['p_key'].isin(keys)]
...
groups = []
sorted_keys = []
for k, v in final_data.groupby('p_key'):
