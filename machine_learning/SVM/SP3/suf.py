...
table_name = "`09ad024f-822f-48e4-9d9e-b5e03c1839a2`.rebalanced_tables.stratified_undersample_fe_training"
df_spark = (
...
if "p_key" in df.columns:
    groups = df["p_key"].map(group_from_pkey)
else:
    groups = pd.Series(["__nogroup__"] * len(df))

id_like = {"Tc_target"}
num_cols = df.select_dtypes(include=["number"]).columns.tolist()
feat_cols = [c for c in num_cols if c not in id_like]

print(f"[Data] rows={len(df)} features={len(feat_cols)}")
...
for cand in ["Tc_r_date", "Wagon_RecordingDate"]:
    if cand in df.columns:
        ...
        break
if time_col is None:
    raise ValueError("No suitable time column found (Tc_r_date or Wagon_RecordingDate)")

if not np.issubdtype(df[time_col].dtype, np.datetime64):
    try:
        df[time_col] = pd.to_datetime(df[time_col])
    except Exception as e:
        raise ValueError(f"{time_col} cannot be converted to datetime: {e}")
...
X_train, X_test = df.iloc[train_idx][feat_cols], df.iloc[test_idx][feat_cols]
y_train, y_test = y.iloc[train_idx], y.iloc[test_idx]

print(f"[Split] Time split: train={len(train_idx)} | test={len(test_idx)} | cutoff={cutoff}")
...
groups_train = groups.iloc[train_idx] if isinstance(groups, pd.Series) else pd.Series(group_ids[train_idx])

inner_gss = GroupShuffleSplit(n_splits=3, test_size=0.2, random_state=42)
candidate_C = [0.25, 0.5, 1.0, 2.0, 5.0, 10.0, 20.0]

best_C, best_score = None, -np.inf
for C in candidate_C:
    fold_scores = []
    for inner_tr, inner_va in inner_gss.split(X_train, y_train, groups=groups_train):
        X_tr, X_va = X_train.iloc[inner_tr], X_train.iloc[inner_va]
        y_tr, y_va = y_train.iloc[inner_tr], y_train.iloc[inner_va]
...
pipe = Pipeline(steps=[
    ("imp", SimpleImputer(strategy="median")),
    ("scaler", StandardScaler()),
    ("svm", LinearSVC(C=best_C, dual=False, class_weight="balanced",
                      max_iter=20000, tol=1e-3, random_state=42))
])
