...
df_spark = (
    spark.table("`09ad024f-822f-48e4-9d9e-b5e03c1839a2`.predictive_maintenance_uofa_2025.preprocess_training_table")
    .orderBy(F.rand(42))
    .limit(200000)
)
...
leak_cols = [
    ...
    "Tc_r_date", "Wagon_RecordingDate"
]
id_cols = ["Tc_BaseCode", "Tc_BaseCode_Mapped", "Tc_SectionBreakStartKM"]  

num_cols = df.select_dtypes(include="number").columns.tolist()
feat_cols = [c for c in num_cols if c not in set(leak_cols + id_cols + ["Tc_target"])]
...
groups = df["Tc_BaseCode"].astype(str) + "_" + df["Tc_SectionBreakStartKM"].astype(str)
...
pipe = Pipeline(steps=[
    ("scaler", StandardScaler()),
    ("svm",    LinearSVC(C=10.0,dual=False, class_weight="balanced",
...
if explain_LIME:
    ...
    pipe.steps[-1] = ('svm', CalibratedClassifierCV(pipe.named_steps['svm'], cv=3))
    pipe.fit(X_train, y_train)
    ...
    # Prediction function returning probability
    ...
    exp = explainer.explain_instance(instance, lime_predict_fn, num_samples=1000, num_features=len(feat_cols))

    # Extract feature contributions
    feat_weights = dict(exp.as_list())

    # Feature importances bar chart
    feat_names = list(feat_weights.keys())
    feat_vals  = [abs(v) for v in feat_weights.values()]

    plt.figure(figsize=(10,4))
    plt.bar(range(len(feat_names)), feat_vals)
    plt.xticks(range(len(feat_names)), feat_names, rotation=45, ha='right')
    plt.title(f'[LIME] Absolute feature importance for instance {idx}')
...
if explain_LIME:
    def explain_multiple_instances(X_test_df, num_samples=500, n_instances=20):
        n_instances = min(n_instances, len(X_test_df))
        agg = np.zeros((n_instances, len(feat_cols)), dtype=np.float32)
        for i, idx in enumerate(range(n_instances)):
            inst = X_test_df.iloc[idx].values
            e = explainer.explain_instance(inst, lime_predict_fn, num_samples=num_samples, num_features=len(feat_cols))
            m = np.zeros(len(feat_cols))
            for f, w in e.as_list():
                m[feat_cols.index(f)] = abs(w)
            agg[i,:] = m
        return agg

    avg_imp = explain_multiple_instances(X_test, n_instances=20, num_samples=500)

    plt.figure(figsize=(10,6))
    plt.imshow(avg_imp.T, aspect='auto', cmap='viridis')
    plt.yticks(ticks=np.arange(len(feat_cols)), labels=feat_cols)
    plt.xlabel('Test instances')
    plt.ylabel('Features')
    plt.title('[LIME] Absolute feature importance for first 20 samples')
