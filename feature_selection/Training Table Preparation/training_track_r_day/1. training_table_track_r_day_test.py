...
# ========== 1) Read tables ==========
tc  = spark.table("dev_adlunise.predictive_maintenance_uofa_2025.trainingcontext")
...
tc_df = (
  tc
  .filter(col("r_date").isNotNull() & col("SectionBreakStartKM").isNotNull())
  .withColumn("Tc_r_date_minus_30", F.date_sub(F.col("r_date"), 30))
  ...
  .select(
      ...
      F.col("r_date").alias("Tc_r_date"),
      F.col("break_date").alias("Tc_break_date"),
      F.col("last_fail_if_available_otherwise_null").alias("Tc_last_fail_if_available_otherwise_null"),
      F.col("rul").alias("Tc_rul"),
      F.col("target").alias("Tc_target"),
      "km_range_start", 
      "km_range_end",
      "Tc_r_date_minus_30"
  )
  .withColumn(
      "Tc_window_end",F.col("Tc_r_date")
  )
  .filter(F.col("Tc_r_date") >= F.lit("2018-12-31"))
  # .filter(F.col("Tc_r_date") == F.lit("2020-05-02"))
  # .filter(F.col("Tc_BaseCode") == F.lit("ARTC-12"))
  # .filter(F.col("Tc_break_date") == F.lit("2020-07-01"))
  # .filter(F.col("Tc_SectionBreakStartKM") == F.lit("224.63"))
)
...
# ========== 3) Wagon pre-aggregation: downscale by “day × 0.02km × BaseCode (mapped/original)” ==========
w_df = (
  w.filter(col("RecordingDate").isNotNull() & col("SectionBreakStartKM").isNotNull())
  #  .filter(F.col("BaseCode") == F.lit("ARTC-12"))
  #  .filter(F.col("SectionBreakStartKM") == F.lit("224.64"))
   .join(bcm, on="BaseCode", how="left")
   .groupBy(
...
# ========== 4) First do coarse date trimming to reduce w size ==========
# Trim w using tc’s global min/max window boundaries (to avoid full table scan)

tc_bounds = tc_df.select(
    F.min("Tc_r_date_minus_30").alias("min_d"),
...
cond = (
    ...
    (w_df.Wagon_RecordingDate >= tc_df.Tc_r_date_minus_30) &
...
# Since Tonnage is usually sparser, first coarse-trim by BaseCode and date, then do interval overlap
joined_for_tng = (
  joined
...
df_result = (
  ...
  .groupBy(
      ...
      "Tc_SectionBreakStartKM", "Tc_break_date", "Tc_last_fail_if_available_otherwise_null",
      "Tc_r_date", "Tc_rul", "Tc_target",
      "Wagon_RecordingDate"
    )
    .agg(
...
# MAGIC %sql
# MAGIC SELECT * FROM dev_adlunise.predictive_maintenance_uofa_2025.railbreaklocations;
