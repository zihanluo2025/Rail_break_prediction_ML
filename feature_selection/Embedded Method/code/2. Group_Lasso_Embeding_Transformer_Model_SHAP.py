...
# MAGIC %pip install pandas scikit-learn mlflow-skinny[databricks] lightgbm
# MAGIC %pip install shap==0.43.0 matplotlib==3.7.1
...
# 2) Select all numeric Wagon_* features
...
# 4) Fill feature nulls with 0 first (to avoid aggregates staying null when all values are null)
...
# # 5) Build aggregation expressions (use mean for features)
# agg_exprs = []
# for c in feature_cols:
#     # Use mean by default
#     agg_exprs.append(F.avg(c).alias(c))

#     # If it's an Acc feature, additionally add min, max, std
#     # if "Acc" in c:
...
agg_exprs = []
for c in feature_cols:
    if "Acc" in c and "RMS" not in c:  
        # Acc-related features → use P95
        agg_exprs.append((F.expr(f"percentile_approx({c}, 0.95)")).alias(c))
        agg_exprs.append(F.coalesce(F.stddev(c), F.lit(0.0)).alias(f"{c}_std"))
        agg_exprs.append(F.coalesce(F.max(c), F.lit(0.0)).alias(f"{c}_max"))
    else:
        # Other features → use mean by default
        agg_exprs.append(F.avg(c).alias(c))

# 6) Aggregate only by specified keys
aggregated_df = (
    df.groupBy(
        "Tc_BaseCode",
        "Tc_BaseCode_Mapped",
...
# 1. Acceleration-related
aggregated_df = aggregated_df.withColumn(
    "Wagon_Acc_total_energy",
    (F.col("Wagon_Acc1_max")**2 + F.col("Wagon_Acc2_max")**2 + 
     F.col("Wagon_Acc3_max")**2 + F.col("Wagon_Acc4_max")**2)
)

aggregated_df = aggregated_df.withColumn(
    "Wagon_Acc_imbalance",
    F.greatest("Wagon_Acc1", "Wagon_Acc2", "Wagon_Acc3", "Wagon_Acc4") - 
    F.least("Wagon_Acc1", "Wagon_Acc2", "Wagon_Acc3", "Wagon_Acc4")
)

aggregated_df = aggregated_df.withColumn(
    "Wagon_Acc_variance_ratio",
    F.col("Wagon_Acc1_std") / (F.col("Wagon_Acc2_std") + 1e-6)
)
...
feature_cols = [c for c in aggregated_df.columns if c.startswith("Wagon_") and c not in EXCLUDE_FEATURES]
...
# Final feature list (without any temporal_features)
all_features = feature_cols
print(f"all features count: {len(all_features)}")

display(aggregated_df.select(*sample_cols).limit(200))
...
class AdaptiveFocalLoss(nn.Module):
    """
    Adaptive Focal Loss - dynamically adjusts alpha and gamma
    """
    def __init__(self, initial_alpha=0.25, initial_gamma=2.0, adapt_rate=0.01):
        super().__init__()
        # Key difference 1: make the parameters learnable via nn.Parameter
        self.alpha = nn.Parameter(torch.tensor(initial_alpha))
        self.gamma = nn.Parameter(torch.tensor(initial_gamma))
        self.adapt_rate = adapt_rate
        
    def forward(self, inputs, targets):
        # Key difference 2: dynamically constrain parameter ranges
        alpha = torch.sigmoid(self.alpha) * 0.8 + 0.1  # [0.1, 0.9]
        gamma = torch.relu(self.gamma) + 0.5  # [0.5, inf)
        
        bce_loss = TF.binary_cross_entropy_with_logits(inputs, targets, reduction='none')
        pt = torch.exp(-bce_loss)
        
        # Key difference 3: account for the positive-sample ratio in the current batch
...
class GroupLassoTransformerModel(nn.Module):
    ...
    def __init__(self,
        ...
        self.prior_pos_prob = prior_pos_prob
            ...
            if isinstance(m, nn.Linear):
                ...
                if m.bias is not None:
                    if hasattr(m, 'out_features') and m.out_features == 1:
                        if self.prior_pos_prob is not None:
                            prior_logit = np.log(self.prior_pos_prob / (1 - self.prior_pos_prob + 1e-8))
                            nn.init.constant_(m.bias, prior_logit)
                        else:
                            nn.init.constant_(m.bias, -2.2)
                    else:
...
class TorchModelForSHAP:
    """
    Wrapper for SHAP - wrap your Transformer model into a SHAP-compatible format
    """
    ...
    def __call__(self, X):
        """
        X: numpy array of shape (N, T, F) or (N, T*F) if flattened
        Returns: numpy array of shape (N,) - probabilities
        """
        # Handle different input shapes
        original_shape = X.shape
        
        # If 2D (flattened), need to determine T and F
        if X.ndim == 2:
            # Assume X is (N, T*F), need to reshape
            # We'll treat it as (N, T=1, F)
            N, total_features = X.shape
            X_3d = X[:, np.newaxis, :]  # (N, 1, F)
        else:
            X_3d = X
        
        # Predict
        with torch.no_grad():
            x_tensor = torch.tensor(X_3d, dtype=torch.float32, device=self.device)
...
class FastPandasDataset(Dataset):
    ...
    def __init__(self, df, feature_cols, label_col='Tc_target',
        ...
        if means is not None and stds is not None:
            m_arr = np.asarray(means).reshape(-1)      # unify shape to (F,)
            s_arr = (np.asarray(stds).reshape(-1) + 1e-6)
            # Directly use training statistics; do not recompute
            for i in range(F):
                feature_data = all_samples[:, :, i]
                t_mean = m_arr[i]
                t_std  = s_arr[i]
                if t_std > 10 or abs(t_mean) > 10:     # same trigger condition as during training
                    feature_data = (feature_data - t_mean) / t_std
                    all_samples[:, :, i] = feature_data
            # Save training stats (reshape back to (F,) for passing/storing)
            self.means = m_arr.astype(np.float32)
            self.stds  = s_arr.astype(np.float32)
        else:
            # Training stats not provided: compute on this set and save them
            means_list, stds_list = [], []
            for i in range(F):
                feature_data = all_samples[:, :, i]
                cur_mean = float(np.mean(feature_data))
                cur_std  = float(np.std(feature_data) + 1e-6)
                if cur_std > 10 or abs(cur_mean) > 10:
                    feature_data = (feature_data - cur_mean) / cur_std
                    all_samples[:, :, i] = feature_data
                means_list.append(cur_mean)
                stds_list.append(cur_std)
            self.means = np.array(means_list, dtype=np.float32)   # (F,)
            self.stds  = np.array(stds_list,  dtype=np.float32)   # (F,)


        # Clipping
        self.clip_los, self.clip_his = None, None
        if clip_mode == 'percentile':
            if clip_lo is not None and clip_hi is not None:
                # Reuse training thresholds (accepts (F,) or (1,1,F))
                lo_arr = np.asarray(clip_lo).reshape(-1)
                hi_arr = np.asarray(clip_hi).reshape(-1)
                for i in range(F):
                    lo, hi = lo_arr[i], hi_arr[i]
                    if lo < hi:
                        feat = all_samples[:, :, i]
                        all_samples[:, :, i] = np.clip(feat, lo, hi)
                self.clip_los = lo_arr.astype(np.float32)
                self.clip_his = hi_arr.astype(np.float32)
            else:
                # Training stats not provided: compute on this set and save them
                ...
                for i in range(F):
                    ...
                    if lo < hi:
                        all_samples[:, :, i] = np.clip(feat, lo, hi)
                    clip_los.append(lo); clip_his.append(hi)
                self.clip_los = np.array(clip_los, dtype=np.float32)  # (F,)
                self.clip_his = np.array(clip_his, dtype=np.float32)  # (F,)
    ...
    if threshold is None:
        try:
            ...
            if len(unique_classes) < 2:
                print(f"Warning: only one class {unique_classes}; using default threshold 0.5")
                threshold = 0.5
            else:
...
prefix_dict_no_group = {
    ...
    "Wagon_Acc_total_energy":  ["Wagon_Acc_total_energy"],
    "Wagon_Acc_imbalance":  ["Wagon_Acc_imbalance"],
    "Wagon_Acc_variance_ratio":  ["Wagon_Acc_variance_ratio"],
...
def filter_feature_groups(selected_features, prefix_dict_with_group):
    """
    Based on the hard-coded prefix_dict_with_group,
    keep only the features that appear in selected_features.
    """
    new_groups = {}
    ...
    for group_name, prefix_list in prefix_dict_with_group.items():
        ...
        for feat in selected_features:
            if feat in prefix_list:  # exact match
...
def compute_shap_values(model, background_data, explain_data, feature_names, 
                       nsamples=100, device='cpu'):
    """
    Compute SHAP values - use KernelExplainer (general but slower)
    
    Parameters:
        model: trained model
        background_data: (N_bg, T, F) background data
        explain_data: (N_exp, T, F) data to explain
        feature_names: list of str, feature names
        nsamples: int, number of SHAP samples (larger = more accurate but slower)
        device: 'cpu' or 'cuda'
    
    Returns:
        shap_values_3d: (N_exp, T, F) SHAP values
    ...
    # Reshape back to 3D: (N_exp, T, F)
    shap_values_3d = shap_values.reshape(N_exp, T, F)
    ...
    return shap_values_3d, explainer.expected_value, explainer

# COMMAND ----------

def visualize_shap_results(shap_values_3d, explain_data, explain_labels, 
                          feature_names, base_value, 
                          top_n=100):
    """
    Generate SHAP visualization results
    
    Parameters:
        shap_values_3d: (N, T, F) SHAP values
        explain_data: (N, T, F) original data
        explain_labels: (N,) ground-truth labels
        feature_names: list, feature names
        base_value: float, baseline value
        top_n: int, display top N features
    """
    ...
    # ============================================
    # 1. Feature Importance (aggregate across all timesteps)
    # ============================================
    print("\n1. Computing feature importance...")
    
    # For each feature, compute the mean of |SHAP| (across samples and time)
    shap_importance = np.abs(shap_values_3d).mean(axis=(0, 1))  # (F,)
    ...
    print(f"\nTop {min(50, len(feature_names))} Important Features:")
    print(feature_importance_df.head(100).to_string(index=False))
    
    
    # Plot Feature Importance
    plt.figure(figsize=(12, 8))
    display_n = min(top_n, len(feature_names))
    y_pos = np.arange(display_n)
    plt.barh(y_pos, feature_importance_df.head(display_n)['importance'].values, 
             color='steelblue')
    plt.yticks(y_pos, feature_importance_df.head(display_n)['feature'].values)
    plt.xlabel('Mean |SHAP value|', fontsize=12)
    plt.title(f'Top {display_n} Feature Importance (SHAP)', fontsize=14, fontweight='bold')
    plt.tight_layout()
    ...
    # Flatten to 2D for the summary plot
    shap_flat = shap_values_3d.mean(axis=1)  # average over time dimension first: (N, F)
    data_flat = explain_data.mean(axis=1)    # (N, F)
    ...
    # ============================================
    # 3. Waterfall Plots (one positive and one negative sample)
    # ============================================
    print("\n3. Creating waterfall plots...")
    
    pos_idx = np.where(explain_labels == 1)[0]
    neg_idx = np.where(explain_labels == 0)[0]
    
    # Positive sample
    if len(pos_idx) > 0:
        sample_idx = pos_idx[0]
        sample_shap = shap_values_3d[sample_idx].mean(axis=0)  # average over timesteps: (F,)
        sample_data = explain_data[sample_idx].mean(axis=0)    # (F,)
    ...
    # Negative sample
    if len(neg_idx) > 0:
        sample_idx = neg_idx[0]
        sample_shap = shap_values_3d[sample_idx].mean(axis=0)
        sample_data = explain_data[sample_idx].mean(axis=0)
...
def run_shap_analysis(model, test_dataset, feature_names, 
                     n_background=50, n_explain=30, nsamples=100,
                     device='cpu'):
    """
    Full SHAP analysis workflow
    
    Parameters:
        model: trained model
        test_dataset: FastPandasDataset object
        feature_names: list of str, feature names
        n_background: int, number of background samples
        n_explain: int, number of samples to explain
        nsamples: int, number of SHAP samples
        device: 'cpu' or 'cuda'
    
    Returns:
        shap_values: (N_explain, T, F) SHAP values
        feature_importance_df: DataFrame, feature importance
        explainer: SHAP explainer object
    """
    print("\n" + "="*70)
    print(" "*20 + "SHAP MODEL EXPLANATION")
    print("="*70)
    ...
    return shap_values, feature_importance_df, explainer
...
def train_loop(model, train_loader, valid_loader, 
    ...
    best_valid_auc_pr, best_model_state, best_metrics = 0, None, None
    ...
    for epoch in range(n_epochs):
        ...
        if (epoch + 1) % 5 == 0 or epoch == 0:
            ...
            print(f"\nTrain metrics:")
            print(f"  AUC-PR: {train_metrics['pr_auc']:.4f}, F1: {train_metrics['f1']:.4f}, Accuracy: {train_metrics['accuracy']:.4f},Precision: {train_metrics['precision']:.4f}, "
                  f"Recall: {train_metrics['recall']:.4f}")
            print(f"\nValid metrics:")
            print(f"  AUC-PR: {valid_metrics['pr_auc']:.4f}, F1: {valid_metrics['f1']:.4f}, Accuracy: {valid_metrics['accuracy']:.4f},Precision: {valid_metrics['precision']:.4f}, "
                  f"Recall: {valid_metrics['recall']:.4f}")
            
        # Key change: early stopping is based on AUC-PR rather than F1
        if valid_metrics['pr_auc'] > best_valid_auc_pr:
            best_valid_auc_pr = valid_metrics['pr_auc']
            best_model_state = model.state_dict().copy()
            best_metrics = valid_metrics.copy()  
            best_epoch = epoch + 1
            patience_counter = 0
            print(f"*** New best valid AUC-PR: {best_valid_auc_pr:.4f} ***")
        else:
            patience_counter += 1
            if patience_counter >= patience:
                print(f"\nEarly stopping: no AUC-PR improvement in {patience} epochs")
                print(f"Best model from epoch {best_epoch}")
                break

        # Overfitting check
        if epoch > 10:
            auc_pr_gap = train_metrics['pr_auc'] - valid_metrics['pr_auc']
            if auc_pr_gap > 0.15:  # AUC-PR gap too large
                print(f"Warning: severe overfitting detected (AUC-PR gap: {auc_pr_gap:.4f})")
    ...
    return model, best_valid_auc_pr, best_metrics, history, best_epoch
...
def make_balanced_sampler(
    ...
    """
    Constructs a WeightedRandomSampler for DataLoader to achieve batch-wise class balance (desired ratio).

    Parameters
    ----
    dataset : torch.utils.data.Dataset
        Must have dataset.y (0/1 labels), shape [N].
    target_pos_fraction : float, default=0.2
        Target positive class fraction (0~1), for example, 0.2 means 20% positive samples.
    replacement : bool, default=True
        Whether sampling is with replacement (recommended True for imbalanced positive samples).
    num_samples : int, optional
        Number of samples to draw per epoch, defaults to len(dataset).
    seed : int, default=42
    ...
    if N_pos == 0:
        raise ValueError("No positive samples in the training set; cannot resample.")

    # Compute positive-class weight factor alpha
    ...
    if verbose:
        expected_pos = target_pos_fraction * 100
        actual_pos = (alpha * N_pos) / (alpha * N_pos + N_neg) * 100
        print(f"[make_balanced_sampler] "
              f"Target positive ratio={expected_pos:.1f}%, expected actual≈{actual_pos:.1f}% "
              f"(Pos={N_pos}, Neg={N_neg}, alpha={alpha:.3f})")
...
def train_model_with_group_lasso(train_df, valid_df, feature_cols, 
    ...
    train_dataset = FastPandasDataset(train_df, feature_cols_clean,label_col='Tc_target',
                                  ...
                                  clip_percentile=(0.1, 99.9),clip_fixed=(-10.0, 10.0))
    ...
    valid_dataset = FastPandasDataset(valid_df, feature_cols_clean,label_col='Tc_target',
                                  ...
                                  clip_lo=train_dataset.clip_los,
                                  clip_hi=train_dataset.clip_his
    )
    ...
    sampler = make_balanced_sampler(train_dataset, target_pos_fraction=0.75)
    ...
    pos_rate = float(pos_count / (pos_count + neg_count))
    ...
    if use_focal_loss:
        ...
        criterion = AdaptiveFocalLoss(initial_alpha=0.25, initial_gamma=0.5)
        print("Using AdaptiveFocalLoss (initial_alpha=0.25, initial_gamma=0.5)")
    else:
        # Weighted BCE
        if use_class_weights:
            pos_weight_value = max(1.0, neg_count / max(1, pos_count)) 
            ...
            print("Not Using use_class_weights with BCEWithLogitsLoss, pos_weight: {pos_weight}")
    ...
    optimizer = torch.optim.AdamW([{"params": decay, "weight_decay": 1e-4},{"params": no_decay, "weight_decay": 0.0}],lr=lr, betas=(0.9, 0.99))   
    ...
    # First training
    model, best_valid_auc_pr, best_metrics, history, best_epoch = train_loop(
    ...
    if retrain_with_selected and len(selected_features) > 0:
        ...
        model, best_valid_auc_pr, best_metrics, history, selected_features = train_model_with_group_lasso(train_df, 
                                     valid_df, 
                                     selected_features,
                                     feature_groups=None,
                                     n_epochs=n_epochs, 
    ...
    print(f"Best AUC-PR: {best_valid_auc_pr:.4f}") 
    print_detailed_metrics(best_metrics)


    if history['train_metrics']:
        final_train_auc_pr = history['train_metrics'][-1]['pr_auc']
        final_valid_auc_pr = history['valid_metrics'][-1]['pr_auc']
        gap = final_train_auc_pr - final_valid_auc_pr
        print(f"\nOverfitting analysis:")
        print(f"  Final Train AUC-PR: {final_train_auc_pr:.4f}")
        print(f"  Final Valid AUC-PR: {final_valid_auc_pr:.4f}")
        print(f"  Gap: {gap:.4f}")
        if gap < 0.05:
            print("  Status: !!!Good")
        elif gap < 0.1:
    ...
    return model, best_valid_auc_pr, best_metrics, history, selected_features
    ...
    def _build_3d_from_df(self, model_input_df: pd.DataFrame):
        ...
        if all_samples.size > 0:
            ...
            if self.means is not None and self.stds is not None:
                m = self.means.reshape(1,1,F)
                s = (self.stds.reshape(1,1,F) + 1e-6)
                all_samples = (all_samples - m) / s
            else:
                print("No means/stds provided; NOT standardizing at inference.")


            # (5) Clipping (consistent with training)
            if self.clip_mode == "percentile":
                if (self.clip_lo is not None) and (self.clip_hi is not None):
                    lo = self.clip_lo.reshape(1,1,F)
                    hi = self.clip_hi.reshape(1,1,F)
                    all_samples = np.clip(all_samples, lo, hi)
                else:
...
def evaluate_and_register_model(model, train_dataset, test_loader, test_df, valid_loader, feature_cols, model_name, 
    ...
    with mlflow.start_run() as run:
        ...
        sample_input = test_df.select("Tc_BaseCode","Tc_BaseCode_Mapped","Tc_SectionBreakStartKM","Tc_date","sample_id","Tc_p_key","Wagon_Twist14m","Wagon_BounceFrt","Wagon_BounceRr","Wagon_BodyRockFrt","Wagon_BodyRockRr","Wagon_LP1","Wagon_LP2","Wagon_LP3","Wagon_LP4","Wagon_Speed","Wagon_BrakeCylinder","Wagon_IntrainForce","Wagon_Acc1","Wagon_Acc2","Wagon_Acc3","Wagon_Acc4","Wagon_Twist2m","Wagon_Acc1_RMS","Wagon_Acc2_RMS","Wagon_Acc3_RMS","Wagon_Acc4_RMS","Wagon_Rail_Pro_L","Wagon_Rail_Pro_R","Wagon_SND","Wagon_VACC","Wagon_VACC_L","Wagon_VACC_R","Wagon_Curvature","Wagon_Track_Offset","Wagon_ICWVehicle","Wagon_SND_L","Wagon_SND_R","Wagon_Acc_total_energy",  "Wagon_Acc_imbalance","Wagon_Acc_variance_ratio").toPandas().fillna(0).head(5)


        sample_output = TorchModelWrapper(model, feature_cols, best_threshold,device="cpu",sample_col="sample_id",pkey_col="Tc_p_key",clip_mode="percentile",clip_percentile=(0.1, 99.9),clip_fixed=(-10.0, 10.0),means=train_dataset.means,stds=train_dataset.stds,clip_lo=train_dataset.clip_los,clip_hi=train_dataset.clip_his).predict(   
            None, sample_input
        )
...
def train_and_register_best_model(train_df, valid_df, test_df, 
                                  ...
                                  model_name="transformer_group_lasso_test_auc_pr_v3_SHAP"):
    """
    ...
    model, best_valid_auc_pr, best_metrics, history, selected_features = train_model_with_group_lasso(
        train_df, valid_df,
        feature_cols,
        feature_groups=None,
        n_epochs=n_epochs,
        batch_size=batch_size,
        lr=2e-4,
        lambda_l2=0.0005,  # L2 regularization (tuned)
        lambda_l1=0.00005,  # L1 regularization (tuned)
        patience=10,
        use_class_weights=False,  # Use class weights for balancing
        use_focal_loss=False,
        retrain_with_selected=False
    )
    ...
    print("\n" + "="*70)
    print("Starting SHAP Analysis on Trained Model")
    print("="*70)

    # Prepare the test dataset (ensure preprocessing consistent with training)
    final_feature_cols = [col for col in all_features 
                        if col not in ['sample_id', '_seq_position', 'Tc_target', 
                                    'has_last_fail', 'days_since_last_fail', 
                                    'Wagon_ICWVehicle', 'has_data', 'doy_cos', 
                                    'doy_sin', 'time_position', 'days_to_target']]

    print(f"Features for SHAP analysis: {len(final_feature_cols)}")

    # Create the test dataset
    test_dataset_for_shap = FastPandasDataset(
        test_df, 
        final_feature_cols,
        label_col='Tc_target',
        sample_col='sample_id',
        seq_col='_seq_position',
        clip_mode='percentile',
        clip_percentile=(0.5, 99.5),
        means=train_dataset.means,  # use training set statistics
        stds=train_dataset.stds,
        clip_lo=train_dataset.clip_los,
        clip_hi=train_dataset.clip_his
    )
    ...
    shap_values, feature_importance_df, shap_explainer = run_shap_analysis(
        model=model,
        test_dataset=test_dataset_for_shap,
        feature_names=final_feature_cols,
        n_background=50,      # number of background samples (tunable)
        n_explain=30,         # number of samples to explain (tunable)
        nsamples=100,         # SHAP sampling count (higher = more accurate but slower)
        device='cpu',         # SHAP recommends CPU
    )

    print("\n" + "="*70)
    print("Top 40 Most Important Features (SHAP)")
    print("="*70)
    ...
    df_result, final_metrics, run_id = evaluate_and_register_model(
        ...
        model_type='transformer_group_lasso_test_auc_pr_v3_SHAP',
        device=device,
        ml_catalog='09ad024f-822f-48e4-9d9e-b5e03c1839a2',
        model_schema_name='feature_selection'
    )
...
def fix_data_split(aggregated_df):
    """
    Use stratified sampling instead of time-based split - fix distribution mismatch
    """
    # Convert to Pandas for stratified sampling
    pdf = aggregated_df.toPandas()
    
    # Stratify by sample_id to keep positive ratio consistent across splits
    samples_df = pdf.groupby('sample_id').agg({
        'Tc_target': 'first'  # label for each sample_id
    }).reset_index()
    
    print(f"Total samples: {len(samples_df)}")
    print(f"Positive rate: {samples_df['Tc_target'].mean():.3f}")
    
    # Stratified sampling
    from sklearn.model_selection import train_test_split
    
    train_samples, temp_samples = train_test_split(
        samples_df, test_size=0.3, random_state=42, 
        stratify=samples_df['Tc_target']
    )
    
    valid_samples, test_samples = train_test_split(
        temp_samples, test_size=0.5, random_state=42,
        stratify=temp_samples['Tc_target']
    )
    
    # Rebuild datasets
    train_df = pdf[pdf['sample_id'].isin(train_samples['sample_id'])]
    valid_df = pdf[pdf['sample_id'].isin(valid_samples['sample_id'])]
    test_df = pdf[pdf['sample_id'].isin(test_samples['sample_id'])]
    
    # Convert back to Spark DataFrame
    train_df = spark.createDataFrame(train_df)
    valid_df = spark.createDataFrame(valid_df)
