...
# MAGIC %md
# MAGIC
# MAGIC # Rebalance_Undersampling_random_fe_training
# MAGIC
...
df = spark.table("`09ad024f-822f-48e4-9d9e-b5e03c1839a2`.predictive_maintenance_uofa_2025.fe_training")
# Step 1: Count the number of minority class records
minority_count = df.filter(F.col("Tc_target") == 1).count()

# Step 2: Randomly sample the majority class
majority_df = df.filter(F.col("Tc_target") == 0)
sampled_majority_df = majority_df.orderBy(F.rand(seed=42)).limit(minority_count)

# Step 3: Keep all minority class records
minority_df = df.filter(F.col("Tc_target") == 1)

# Step 4: Combine the record-level undersampling results
sampled_df_record = sampled_majority_df.union(minority_df)

# Step 5: Optional: Check the distribution
sampled_df_record.groupBy("Tc_target").count().show()
df_result = sampled_df_record
